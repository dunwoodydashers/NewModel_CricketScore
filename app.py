...
def init_db():
    with conn.session as s:
        ...
        # Create matches table without complex formatting
        ...
        # Add the score_state column separately as a plain string
        try:
...
if choice == "Schedule & Rosters":
    ...
    with c1:
        ...
        teams = get_teams()
        sel_t = st.selectbox("Select Team", teams if teams else ["No teams available"])
    ...
    with c2:
        ...
        teams = get_teams()
        ta, tb = st.selectbox("Team A", teams if teams else []), st.selectbox("Team B", teams if teams else [])
